...
class Player():
    ...
    def get_history(self, game):
        ...
        if len(self.history) != 1:
            history = history + "Vous avez déjà visité les pièces suivantes dans cet ordre :\n"
            for room in self.history[:-1]:
                history = history + "-" + room.description.strip("dansdevant ")
            history = history + "\n"
        return history
