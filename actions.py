...
class Actions:
    ...
    def look(game, list_of_words, number_of_parameters):
        ...
        Inventory.get_inventory(game, game.player.current_room)
        return True
    ...
    def take(game, list_of_words, number_of_parameters):
        ...
        if total_weight + game.player.current_room.inventory.get(object).weight > game.player.inventory_weight_max :
            ...
            game.warning = game.warning + "\nLe poids total de ce que vous portez est de {total_weight} g.".format(total_weight=total_weight)
            max_weight = game.player.inventory_weight_max
            game.warning = game.warning + "Le maximum de ce que vous pouvez porter est de {max_weight} g.".format(max_weight=max_weight)
    ...
    def answer (game, list_of_words, number_of_parameters):
        ...
        #Print a message of the character.
            ...
            if player.current_room.characters[pnj] == "barbaaigri":
                game.player.current_room.characters[pnj].msgs.insert(0, "\nMouais, c'est ça. Bon bah là va down mais prend garde au cyclope une fois dans le terrier, j'ai connu il fut un temps un bon monsieur qui s'appellait Ulysse, pense a prende ton mouton en sortant toi aussi.")
            else :
                game.player.current_room.characters[pnj].msgs.insert(0, "\n Tout à fait. Bravo !")
